//
//  YXRemoteResourceLoaderDelegate.m
...
@interface YXRemoteResourceLoaderDelegate ()<YXAudioDownLoaderDelegate>

@property (nonatomic, strong) YXAudioDownLoader *downLoader;

@property (nonatomic, strong) NSMutableArray *loadingRequests;

@end

@implementation YXRemoteResourceLoaderDelegate

- (YXAudioDownLoader *)downLoader {
    if (!_downLoader) {
        _downLoader = [[YXAudioDownLoader alloc] init];
...
- (BOOL)resourceLoader:(AVAssetResourceLoader *)resourceLoader shouldWaitForLoadingOfRequestedResource:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    if ([YXRemoteAudioFile cacheFileExists:url]) {
...
- (void)handleAllLoadingRequest {
    ...
    for (AVAssetResourceLoadingRequest *loadingRequest in self.loadingRequests) {
        ...
        // 2. 填充数据
        NSData *data = [NSData dataWithContentsOfFile:[YXRemoteAudioFile tmpFilePath:url] options:NSDataReadingMappedIfSafe error:nil];
        if (data == nil) {
            data = [NSData dataWithContentsOfFile:[YXRemoteAudioFile cacheFilePath:url] options:NSDataReadingMappedIfSafe error:nil];
        }
...
- (void)handleLoadingRequest:(AVAssetResourceLoadingRequest *)loadingRequest {
    ...
    NSURL *url = loadingRequest.request.URL;
    long long totalSize = [YXRemoteAudioFile cacheFileSize:url];
    loadingRequest.contentInformationRequest.contentLength = totalSize;
    
    NSString *contentType = [YXRemoteAudioFile contentType:url];
    loadingRequest.contentInformationRequest.contentType = contentType;
    loadingRequest.contentInformationRequest.byteRangeAccessSupported = YES;
    
    // 2. 相应数据给外界
    NSData *data = [NSData dataWithContentsOfFile:[YXRemoteAudioFile cacheFilePath:url] options:NSDataReadingMappedIfSafe error:nil];
