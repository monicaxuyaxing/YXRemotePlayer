...
@interface YXAudioDownLoader()<NSURLSessionDataDelegate>
...
@implementation YXAudioDownLoader
...
- (void)cancelAndClean {
    ...
    self.session = nil;
    // 清空本地已经存储的临时缓存
    [YXRemoteAudioFile clearTmpFile:self.url];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler {
    ...
    self.outputStream = [NSOutputStream outputStreamToFileAtPath:[YXRemoteAudioFile tmpFilePath:self.url] append:YES];
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    ...
    if (error == nil) {
        NSURL *url = self.url;
        if ([YXRemoteAudioFile tmpFileSize:url] == self.totalSize) {
            //            移动文件 : 临时文件夹 -> cache文件夹
            [YXRemoteAudioFile moveTmpPathToCachePath:url];
        }
