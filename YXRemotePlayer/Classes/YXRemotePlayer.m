...
@interface YXRemotePlayer ()<NSCopying, NSMutableCopying>
{
    // 标识用户是否进行了手动暂停
    BOOL _isUserPause;
}
...
/**
 资源加载代理
 */
@property (nonatomic, strong) YXRemoteResourceLoaderDelegate *resourceLoaderDelegate;
...
- (void)playWithURL:(NSURL *)url isCache:(BOOL)isCache {
    ...
    self.resourceLoaderDelegate = [YXRemoteResourceLoaderDelegate new];
...
- (void)pause {
    ...
    _isUserPause = YES;
    if (self.player) {
        self.state = YXRemotePlayerStatePause;
    }
}
...
- (void)resume {
    ...
    if (self.player && self.player.currentItem.playbackLikelyToKeepUp) {
        self.state = YXRemotePlayerStatePlaying;
    }
}
...
- (void)stop {
    ...
    self.player = nil;
    if (self.player) {
        self.state = YXRemotePlayerStateStopped;
    }
}
...
/**
 ...
 @param state 状态
 */
- (void)setState:(YXRemotePlayerState)state {
    _state = state;
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"status"]) {
        ...
        if (status == AVPlayerItemStatusReadyToPlay) {
            ...
            NSLog(@"状态未知");
            self.state = YXRemotePlayerStateFailed;
        }
        ...
        if (ptk) {
            ...
            self.state = YXRemotePlayerStateLoading;
        }
...
#pragma mark - 单例方法
static YXRemotePlayer *_shareInstance;
+ (instancetype)shareInstance {
    if (!_shareInstance) {
        _shareInstance = [[YXRemotePlayer alloc] init];
    }
